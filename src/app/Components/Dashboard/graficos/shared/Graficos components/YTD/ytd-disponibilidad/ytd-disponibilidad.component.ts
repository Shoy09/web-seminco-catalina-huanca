...
@Component({
  selector: 'app-ytd-disponibilidad',
  standalone: true,
  imports: [NgxEchartsDirective],
  providers: [provideEchartsCore({ echarts })],
  templateUrl: './ytd-disponibilidad.component.html',
  styleUrl: './ytd-disponibilidad.component.css',
})
export class YtdDisponibilidadComponent implements OnChanges {
  @Input() data: any[] = [];
  chartOptions: any = {};
  ...
  actualizarGrafico(): void {
    ...
    const valores = datos.map(d => Number(d.disponibilidad || 0));
      ...
      tooltip: {
        ...
        formatter: (params: any) => {
          ...
          return `<strong>${item.periodo || item.key}</strong><br/><hr style="margin:5px 0"/>
            Disponibilidad: <b>${Number(item.disponibilidad || 0).toFixed(2)} %</b>`;
        },
      },
