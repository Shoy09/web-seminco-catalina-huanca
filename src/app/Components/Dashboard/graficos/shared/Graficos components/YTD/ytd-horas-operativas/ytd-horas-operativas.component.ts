...
@Component({
  selector: 'app-ytd-horas-operativas',
  standalone: true,
  imports: [NgxEchartsDirective],
  providers: [provideEchartsCore({ echarts })],
  templateUrl: './ytd-horas-operativas.component.html',
  styleUrl: './ytd-horas-operativas.component.css',
})
export class YtdHorasOperativasComponent implements OnChanges {
  @Input() data: any[] = [];
  chartOptions: any = {};
  ...
  actualizarGrafico(): void {
    ...
    const valores = datos.map(d => Number(d.horasOperativas || 0));
    ...
    this.chartOptions = {
      tooltip: {
        ...
        formatter: (params: any) => {
          ...
          return `<strong>${item.periodo || item.key}</strong><br/><hr style="margin:5px 0"/>
            Hrs Operativas: <b>${Number(item.horasOperativas || 0).toFixed(2)} h</b>`;
        },
      },
      ...
      series: [{
        ...
        label: { show: true, position: 'top', formatter: (p: any) => `${Number(p.value).toFixed(1)}h`, fontWeight: 'bold', fontSize: 9, color: CHART_COLORS.grey },
