...
@Component({
  selector: 'app-ytd-equipos-utilizados',
  standalone: true,
  imports: [NgxEchartsDirective],
  providers: [provideEchartsCore({ echarts })],
  templateUrl: './ytd-equipos-utilizados.component.html',
  styleUrl: './ytd-equipos-utilizados.component.css',
})
export class YtdEquiposUtilizadosComponent implements OnChanges {
  @Input() data: any[] = [];
  chartOptions: any = {};
  ...
  actualizarGrafico(): void {
    ...
    const valores = datos.map(d => Number(d.equiposUtilizados || 0));
    const maxValor = Math.max(...valores, 1);
    const escalaMax = Math.ceil(maxValor / 1) * 1;
    ...
    this.chartOptions = {
      tooltip: {
        ...
        formatter: (params: any) => {
          ...
          return `<strong>${item.periodo || item.key}</strong><br/><hr style="margin:5px 0"/>
            Equipos utilizados: <b>${Number(item.equiposUtilizados || 0).toFixed(2)} </b>`;
        },
      },
      ...
      yAxis: { type: 'value', min: 0, max: escalaMax, axisLabel: { formatter: `{value} `, fontSize: 10, color: CHART_COLORS.grey }, splitLine: CHART_SPLIT_LINE },
      dataZoom: [
      ...
      series: [{
        ...
        data: valores.map(v => ({ value: v, itemStyle: { color: CHART_COLORS.highlightOrange } })),
        itemStyle: { borderRadius: [6, 6, 0, 0], ...CHART_BAR_SHADOW },
        label: { show: true, position: 'top', formatter: (p: any) => `${Number(p.value).toFixed(1)}`, fontWeight: 'bold', fontSize: 9, color: CHART_COLORS.grey },
