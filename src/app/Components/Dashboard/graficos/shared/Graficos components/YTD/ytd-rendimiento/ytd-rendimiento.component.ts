...
@Component({
  selector: 'app-ytd-rendimiento',
  standalone: true,
  imports: [NgxEchartsDirective],
  providers: [provideEchartsCore({ echarts })],
  templateUrl: './ytd-rendimiento.component.html',
  styleUrl: './ytd-rendimiento.component.css',
})
export class YtdRendimientoComponent implements OnChanges {
  @Input() data: any[] = [];
  chartOptions: any = {};
  ...
  actualizarGrafico(): void {
    ...
    const valores = datos.map(d => Number(d.rendimiento || 0));
    ...
    this.chartOptions = {
      tooltip: {
        ...
        formatter: (params: any) => {
          ...
          return `<strong>${item.periodo || item.key}</strong><br/><hr style="margin:5px 0"/>
            Rendimiento: <b>${Number(item.rendimiento || 0).toFixed(2)} m/h</b>`;
        },
      },
      ...
      yAxis: { type: 'value', min: 0, max: escalaMax, axisLabel: { formatter: `{value} m/h`, fontSize: 10, color: CHART_COLORS.grey }, splitLine: CHART_SPLIT_LINE },
      dataZoom: [
      ...
      series: [{
        ...
        label: { show: true, position: 'top', formatter: (p: any) => `${Number(p.value).toFixed(1)}m/h`, fontWeight: 'bold', fontSize: 9, color: CHART_COLORS.grey },
