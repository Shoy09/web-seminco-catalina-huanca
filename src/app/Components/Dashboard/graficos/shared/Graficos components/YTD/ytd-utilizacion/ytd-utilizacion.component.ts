...
@Component({
  selector: 'app-ytd-utilizacion',
  standalone: true,
  imports: [NgxEchartsDirective],
  providers: [provideEchartsCore({ echarts })],
  templateUrl: './ytd-utilizacion.component.html',
  styleUrl: './ytd-utilizacion.component.css',
})
export class YtdUtilizacionComponent implements OnChanges {
  @Input() data: any[] = [];
  chartOptions: any = {};
  ...
  actualizarGrafico(): void {
    ...
    const valores = datos.map(d => Number(d.utilizacion || 0));
    ...
    this.chartOptions = {
      tooltip: {
        ...
        formatter: (params: any) => {
          ...
          return `<strong>${item.periodo || item.key}</strong><br/><hr style="margin:5px 0"/>
            Utilización: <b>${Number(item.utilizacion || 0).toFixed(2)} %</b>`;
        },
      },
      ...
      series: [{
        ...
        data: valores.map(v => ({ value: v, itemStyle: { color: CHART_COLORS.forestGreen } })),
