...
@Component({
  selector: 'app-mtbf-semana',
  standalone: true,
  imports: [NgxEchartsDirective],
  providers: [provideEchartsCore({ echarts })],
  templateUrl: './mtbf-semana.component.html',
  styleUrl: './mtbf-semana.component.css',
})
export class MtbfSemanaComponent implements OnChanges {
  @Input() data: any[] = [];
  chartOptions: any = {};
  ...
  actualizarGrafico(): void {
    ...
    const valores = datos.map(d => Number(d.mtbf || 0));
    ...
    this.chartOptions = {
      tooltip: {
        ...
        formatter: (params: any) => {
          ...
          return `<strong>${item.periodo} ${item.anio || ''}</strong><br/><hr style="margin:5px 0"/>
            MTBF: <b>${Number(item.mtbf || 0).toFixed(2)} h</b><br/>
            MTTR: ${Number(item.mttr || 0).toFixed(2)} h<br/>
      ...
      series: [{
        ...
        data: valores.map(v => ({ value: v, itemStyle: { color: colorPorMTBF(v) } })),
