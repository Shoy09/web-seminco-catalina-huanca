...
export class DiagramaParetoComponent implements OnChanges {
  @Input() data: any[] = [];
  chartOptions: any = {};
  ...
  actualizarGrafico(): void {
    ...
    // Ordenar descendente por horasDemora, excluir sin observación
    const datos = [...this.data]
      .filter(d => {
        const obs = String(d.observacion || d.descripcion || '').trim().toUpperCase();
        return obs && obs !== 'SIN OBSERVACIÓN' && obs !== 'SIN OBSERVACION';
      })
      .sort((a, b) => Number(b.horasDemora || 0) - Number(a.horasDemora || 0));

    if (!datos.length) { this.chartOptions = {}; return; }

    const total = datos.reduce((s, d) => s + Number(d.horasDemora || 0), 0);

    // Calcular acumulado % (ParetoObservacion)
    let acum = 0;
    const paretoObs = datos.map(d => {
      acum += Number(d.horasDemora || 0);
      return total > 0 ? Number(((acum / total) * 100).toFixed(2)) : 0;
    });

    const observaciones = datos.map(d => d.observacion || d.descripcion || 'SIN OBSERVACIÓN');
    const horas = datos.map(d => Number(d.horasDemora || 0));
    ...
    this.chartOptions = {
      tooltip: {
        ...
        formatter: (params: any) => {
          ...
          const h = Number(item.horasDemora || 0);
          const pct = total > 0 ? ((h / total) * 100).toFixed(1) : '0.0';
          const codigos = (item.codigos || []).join(', ');
          const tipo = item.tipoDemora || '';
          return `
            <strong>${item.observacion || item.descripcion || 'SIN OBSERVACIÓN'}</strong><br/>
            <hr style="margin:5px 0"/>
            Horas demora: <b>${h.toFixed(2)} h</b><br/>
            % individual: ${pct}%<br/>
