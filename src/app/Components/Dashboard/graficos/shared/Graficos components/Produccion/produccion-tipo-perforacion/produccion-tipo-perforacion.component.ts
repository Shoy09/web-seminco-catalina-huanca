...
import { CHART_COLORS, CHART_PALETTE, CHART_SPLIT_LINE, CHART_BAR_SHADOW, CHART_AXIS_LABEL } from '../../../../../../../shared/chart-theme';
...
@Component({
  selector: 'app-produccion-tipo-perforacion',
  standalone: true,
  imports: [NgxEchartsDirective],
  providers: [provideEchartsCore({ echarts })],
  templateUrl: './produccion-tipo-perforacion.component.html',
  styleUrl: './produccion-tipo-perforacion.component.css',
})
export class ProduccionTipoPerforacionComponent implements OnChanges {
  @Input() data: any[] = [];
  chartOptions: any = {};
  ...
  actualizarGrafico(): void {
    ...
    const tipos = datos.map(d => d.tipo || 'SIN TIPO');
    ...
    this.chartOptions = {
      tooltip: {
        ...
        formatter: (params: any) => {
          ...
          return `<strong>${item.tipo || 'SIN TIPO'}</strong><br/><hr style="margin:5px 0"/>
      ...
      yAxis: { type: 'category', data: tipos, axisLabel: { fontSize: 11, fontWeight: 'bold', color: CHART_COLORS.grey }, axisTick: { show: false } },
      series: [{
        ...
        data: valores.map((v, i) => {
          ...
          return {
            value: v,
            itemStyle: { color: CHART_PALETTE[i % CHART_PALETTE.length], borderRadius: [0, 6, 6, 0], ...CHART_BAR_SHADOW },
