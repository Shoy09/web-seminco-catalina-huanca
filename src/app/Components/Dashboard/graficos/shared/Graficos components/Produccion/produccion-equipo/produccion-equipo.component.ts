...
@Component({
  selector: 'app-produccion-equipo',
  standalone: true,
  imports: [NgxEchartsDirective],
  providers: [provideEchartsCore({ echarts })],
  templateUrl: './produccion-equipo.component.html',
  styleUrl: './produccion-equipo.component.css',
})
export class ProduccionEquipoComponent implements OnChanges {
  @Input() data: any[] = [];
  chartOptions: any = {};
    ...
    this.chartOptions = {
      tooltip: {
        ...
        formatter: (params: any) => {
          ...
          return `<strong>${item.n_equipo || item.modeloEquipo || 'SIN EQUIPO'}</strong><br/>
      ...
      yAxis: {
        type: 'value', min: 0, max: escalaMax,
        axisLabel: { formatter: '{value} m', ...CHART_AXIS_LABEL },
