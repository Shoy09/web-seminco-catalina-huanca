...
export class RendimientoRankingGuardiaComponent implements OnInit {
  
  chartOptions: any = {};

  // Datos por tipo de guardia - RENDIMIENTO
  readonly datosPorGuardia = [
    { guardia: 'C', valor: 94, color: '#f9e79f' },
    { guardia: 'B', valor: 88, color: '#85c1e9' },
    { guardia: 'A', valor: 91, color: '#85c1e9' }
  ];
    ...
    this.chartOptions = {
      ...
      tooltip: {
        ...
        formatter: (params: any) => {
          const data = params[0];
          let nivel = '';
          if (data.value >= 90) nivel = 'Excelente 🏆';
          else if (data.value >= 75) nivel = 'Bueno 👍';
          else if (data.value >= 60) nivel = 'Regular ⚠️';
          else nivel = 'Requiere mejora 🔧';
          
          return `<strong>Guardia ${data.name}</strong><br/>
                  Rendimiento: ${data.value}%<br/>
                  Nivel: ${nivel}`;
        }
      },
